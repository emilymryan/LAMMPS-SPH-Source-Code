...
PairSPHTwospeciesIsodiffusionMigrationCSRbvn::PairSPHTwospeciesIsodiffusionMigrationCSRbvn(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  DA = atom->dvector[iDA];
  ...
  if (iDC < 0)
    error->all(FLERR,
	       "Can't find property DC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  DC = atom->dvector[iDC];
  ...
  if (iRC < 0)
    error->all(FLERR,
               "Can't find property RC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  RC = atom->dvector[iRC];

  //find the curvature coefficient nucleates
  int fk;
  int ik = atom->find_custom("k", fk);
  if (ik < 0)
    error->all(FLERR,
               "Can't find property k for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  k = atom->dvector[ik];

  //find the surface energy nucleates                                                                                                                                    
  int fsurf_energy;
  int isurf_energy = atom->find_custom("surf_energy", fsurf_energy);
  if (isurf_energy < 0)
    error->all(FLERR,
               "Can't find property surf_energy for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  surf_energy = atom->dvector[isurf_energy];

  // find the simulation time
  int ftime;
  int itime = atom->find_custom("time",ftime);
  if (itime < 0)
    error->all(FLERR,
	       "Can't find property time for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  time = atom->dvector[itime];
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  mM = atom->dvector[imM];
  ...
  if (idmM < 0)
    error->all(FLERR,
	       "Can't find property dmM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  ny = atom->dvector[iny];
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
  nz = atom->dvector[inz];
...
PairSPHTwospeciesIsodiffusionMigrationCSRbvn::~PairSPHTwospeciesIsodiffusionMigrationCSRbvn() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1)) { // fluid-fluid interaction outside of flux
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    deltaDcC = 1.0*RC[j]*bulkcA*bulkcC*(((cC[i]/cC_init)*exp(transCoefC*faraday*overPot/R/T + surf_energy[j]*k[j]*mVol/R/T)) - ((mM[j]/mass_init)*exp(-1*transCoefM*faraday*overPot/R/T)));
	  ...
	  if (r<= phase_support[itype][jtype]) {
	    ...
	    deltaDmM = (jmass)*RC[i]*bulkcA*bulkcC*(((cC[j]/cC_init)*exp(transCoefC*faraday*overPot/R/T + surf_energy[i]*k[i]*mVol/R/T)) - ((mM[i]/mass_init)*exp(-1*transCoefM*faraday*overPot/R/T)));
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n");
}
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn::coeff(int narg, char **arg) {
  if (narg != 8)
    error->all(FLERR,"Incorrect number of args for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n coefficients");
  if (!allocated)
    allocate();
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/twospecies/isodiffusion/migration/csr/constrc/bv/n");
}
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvn::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/twospecies/isodiffusion/migration/csr/bv/n coeffs are not set");
  }
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvn::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvn::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvn::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
