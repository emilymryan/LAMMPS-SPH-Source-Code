...
void PairSPHSurfaceReactionLangmuirPorous::coeff(int narg, char **arg) {
  ...
  kaA = utils::numeric(FLERR, arg[3],false,lmp);
  kdA = utils::numeric(FLERR, arg[4],false,lmp);
  lambda = utils::numeric(FLERR, arg[5],false,lmp);
  As = utils::numeric(FLERR, arg[6],false,lmp);
  Vp = utils::numeric(FLERR, arg[7],false,lmp);

  // Check if periodicity for transport is allowed
  is_periodic = utils::numeric(FLERR, arg[8],false,lmp);
