...
PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::PairSPHTwospeciesIsodiffusionMigrationCSRbvn2(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  DA = atom->dvector[iDA];
  ...
  if (iDC < 0)
    error->all(FLERR,
	       "Can't find property DC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  DC = atom->dvector[iDC];
  ...
  if (iRC < 0)
    error->all(FLERR,
               "Can't find property RC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  RC = atom->dvector[iRC];
  ...
  if (isurf_energy < 0)
    error->all(FLERR,
	       "Can't find property surf_energy sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ...
  if (irR < 0)
    error->all(FLERR,
               "Can't find property rR for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  rR = atom->dvector[irR];
  ...
  if (idrR < 0)
    error->all(FLERR,
               "Can't find property drR for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  mM = atom->dvector[imM];
  ...
  if (idmM < 0)
    error->all(FLERR,
	       "Can't find property dmM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  ny = atom->dvector[iny];
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
  nz = atom->dvector[inz];
...
PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::~PairSPHTwospeciesIsodiffusionMigrationCSRbvn2() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1)) { // fluid-fluid interaction outside of flux
	...
	else if ((itype==2) && (jtype==2)) {
	  if (r <= phase_support[itype][jtype]) {
	  deltarR = 0.000000425*(j0*mVol/faraday)*((faraday*overPot/R/T)+(2*surf_energy[i]*mVol/rR[i]/R/T));
	  deltarR *= fabs(nx[j]) + fabs(nx[i]) + fabs(ny[j]) + fabs(ny[i]);
	  drR[i] += deltarR*fabs(wfd);	  
	  }
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    deltaDcC = 1.0*RC[j]*bulkcA*bulkcC*(((cC[i]/cC_init)*exp(transCoefC*faraday*overPot/R/T + surf_energy[j]*2*mVol/rR[j]/R/T)) - ((mM[j]/mass_init)*exp(-1*transCoefM*faraday*overPot/R/T)));
	  ...
	  if (r<= phase_support[itype][jtype]) {
	    ...
	    // Butler - Volmer equations for reaction at solid fluid interface
	    deltaDmM = (jmass)*RC[i]*bulkcA*bulkcC*(((cC[j]/cC_init)*exp(transCoefC*faraday*overPot/R/T + surf_energy[i]*2*mVol/rR[i]/R/T)) - ((mM[i]/mass_init)*exp(-1*transCoefM*faraday*overPot/R/T)));
	    printf("jmass: %.10f \n", jmass); 
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2");
}
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::coeff(int narg, char **arg) {
  if (narg != 8)
    error->all(FLERR,"Incorrect number of args for pair_style sph/twospecies/isodiffusion/migration/csr/bv/n2 coefficients");
  if (!allocated)
    allocate();
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/twospecies/isodiffusion/migration/csr/constrc/bv/n2");
}
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/twospecies/isodiffusion/migration/csr/bv/n2 coeffs are not set");
  }
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvn2::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
