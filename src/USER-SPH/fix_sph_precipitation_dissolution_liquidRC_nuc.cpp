...
#include <stdio.h>
#include <string.h>
#include "fix_sph_precipitation_dissolution_liquidRC_nuc.h"
#include <math.h>
#include <stdlib.h>
#include <string.h>
#include "atom.h"
...
FixSPHPrecipitationDissolutionLiquidRCNuc::FixSPHPrecipitationDissolutionLiquidRCNuc(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {

  if ((atom->esph_flag != 1) || (atom->rho_flag != 1))
    error->all(FLERR,
  	       "fix sph/precipitation/dissolution/liquidRC/nuc command requires atom_style with both energy and density, e.g. meso");

  if (narg != 7)
    error->all(FLERR,"Illegal number of arguments for fix sph/precipitation/dissolution/liquidRC/nuc command");
  ...
  if (rRinit <= 0)
    error->all(FLERR,"Illegal value for critical radius");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for fix sph/precipitation/dissolution/liquidRC/nuc");
  cC = atom->dvector[icC];
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for fix sph/precipitation/dissolution/liquidRC/nuc");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for fix sph/precipitation/dissolution/liquidRC/nuc");
  dcA = atom->dvector[idcA];

  // find the radisu property                                                                                                                                            
  int frR;
  int irR = atom->find_custom("rR", frR);
  if (irR < 0)
    error->all(FLERR,
               "Can't find property rR for fix sph/precipitation/dissolution/liquidRC/nuc");
  rR = atom->dvector[irR];
  ...
  if (inN < 0)
    error->all(FLERR,
               "Can't find property nN for fix sph/precipitation/dissolution/liquidRC/nuc");
  nN = atom->dvector[inN];
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for fix sph/precipitation/dissolution/liquidRC/nuc");
  mM = atom->dvector[imM];
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for fix sph/precipitation/dissolution/liquidRC/nuc");
...
FixSPHPrecipitationDissolutionLiquidRCNuc::~FixSPHPrecipitationDissolutionLiquidRCNuc() {
}

/* ---------------------------------------------------------------------- */

int FixSPHPrecipitationDissolutionLiquidRCNuc::setmask() {
  int mask = 0;
  mask |= END_OF_STEP;
  return mask;
}
...
void FixSPHPrecipitationDissolutionLiquidRCNuc::init() {
...
void FixSPHPrecipitationDissolutionLiquidRCNuc::init_list(int, NeighList *ptr)
{
  list = ptr;
}
...
void FixSPHPrecipitationDissolutionLiquidRCNuc::end_of_step()
{
  ...
  for (i = 0; i < nlocal; i++) {
	...
	local_pot[i] = 0.0;
	  ...
	  rR[i] = 0.0; // radius become 0.0 for dissolution
          nN[i] = 0.0;
...
int FixSPHPrecipitationDissolutionLiquidRCNuc::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i, j, m;
  m = 0;
  for (i = 0; i < n; i++) {
...
void FixSPHPrecipitationDissolutionLiquidRCNuc::unpack_forward_comm(int n, int first, double *buf) 
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int FixSPHPrecipitationDissolutionLiquidRCNuc::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  int *type = atom->type;
...
void FixSPHPrecipitationDissolutionLiquidRCNuc::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  int *type = atom->type;
