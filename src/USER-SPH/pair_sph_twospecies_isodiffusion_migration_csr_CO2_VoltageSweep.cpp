...
PairSPHTwospeciesIsodiffusionMigrationCSRCO2::PairSPHTwospeciesIsodiffusionMigrationCSRCO2(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  DA = atom->dvector[iDA];
  ...
  if (iDC < 0)
    error->all(FLERR,
	       "Can't find property DC for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  DC = atom->dvector[iDC];
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  ny = atom->dvector[iny];
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
  nz = atom->dvector[inz];

  // find the current Applied Potential
...
PairSPHTwospeciesIsodiffusionMigrationCSRCO2::~PairSPHTwospeciesIsodiffusionMigrationCSRCO2() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationCSRCO2::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationCSRCO2::compute(int eflag, int vflag) {
 ...
 // Communicate the local cA to the ghost atoms
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1)) { // fluid-fluid interaction outside of flux
	  ...
	  // Applied Potential Calculation - EDIT EQUATION
	  //dPot[i] += initPot;

	} // fluid-fluid interaction
	else if ((itype==1) && (jtype==2)) {  // fluid-solid interaction
	  if (r <= phase_support[itype][jtype]) {
	    //overPot += (sweeprate*dtcA) + applied_pot_init;
	    overPot = applied_pot;
	    // reaction of CO2 at anode surface
	    deltaDcC = 1.0*keq*bulkcA*bulkcC*((cC[i]/cC_init)*exp(transCoefC*faraday*overPot/R/T));
...
void PairSPHTwospeciesIsodiffusionMigrationCSRCO2::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRCO2::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/twospecies/isodiffusion/migration/csr/CO2");
}
...
void PairSPHTwospeciesIsodiffusionMigrationCSRCO2::coeff(int narg, char **arg) {
  if (narg != 7)
    error->all(FLERR,"Incorrect number of args for pair_style sph/twospecies/isodiffusion/migration/csr/CO2 coefficients");
  if (!allocated)
    allocate();
  ...
  keq = utils::numeric(FLERR,arg[4],false,lmp);

  // Get applied potential
  applied_pot = utils::numeric(FLERR,arg[5],false,lmp);
  
  // Get the initial CO2 Concentration
  cC_init = utils::numeric(FLERR,arg[6],false,lmp);

  // Get the initial Applied Pot (EDIT)
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/twospecies/isodiffusion/migration/csr/CO2");
}
...
double PairSPHTwospeciesIsodiffusionMigrationCSRCO2::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/twospecies/isodiffusion/migration/csr/CO2 coeffs are not set");
  }
...
double PairSPHTwospeciesIsodiffusionMigrationCSRCO2::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationCSRCO2::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationCSRCO2::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationCSRCO2::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRCO2::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
