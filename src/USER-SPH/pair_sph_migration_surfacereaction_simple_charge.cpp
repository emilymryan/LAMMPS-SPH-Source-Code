...
PairSPHMigrationSurfaceReactionSimpleCharge::PairSPHMigrationSurfaceReactionSimpleCharge(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/migration/surfacereaction/simple/charge");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/migration/surfacereaction/simple/charge");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (iDAx < 0)
    error->all(FLERR,
	       "Can't find property DAx for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (iDAy < 0)
    error->all(FLERR,
	       "Can't find property DAy for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (iDCx < 0)
    error->all(FLERR,
	       "Can't find property DCx for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (iDCy < 0)
    error->all(FLERR,
	       "Can't find property DCy for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (iRC < 0)
    error->all(FLERR,
	       "Can't find property RC for pair_style sph/migration/surfacereaction/simple/charge");
  RC = atom->dvector[iRC];
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for pair_style sph/migration/surfacereaction/simple/charge");
  mM = atom->dvector[imM];
  ...
  if (idmM < 0)
    error->all(FLERR,
	       "Can't find property dmM for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/migration/surfacereaction/simple/charge");
  ...
  if (inext_local_pot < 0)
    error->all(FLERR,
	       "Can't find property next_local_pot for pair_style sph/migration/surfacereaction/simple/charge");
...
PairSPHMigrationSurfaceReactionSimpleCharge::~PairSPHMigrationSurfaceReactionSimpleCharge() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHMigrationSurfaceReactionSimpleCharge::init_style()
{
...
void PairSPHMigrationSurfaceReactionSimpleCharge::compute(int eflag, int vflag) {
...
void PairSPHMigrationSurfaceReactionSimpleCharge::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHMigrationSurfaceReactionSimpleCharge::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/migration/surfacereaction/simple/charge");
}
...
void PairSPHMigrationSurfaceReactionSimpleCharge::coeff(int narg, char **arg) {
  if (narg != 9)
    error->all(FLERR,"Incorrect number of args for pair_style sph/migration/surfacereaction/simple/charge  coefficients");
  if (!allocated)
    allocate();
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/migration/surfacereaction/simple/charge");
}
...
double PairSPHMigrationSurfaceReactionSimpleCharge::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/migration/surfacereaction/simple/charge coeffs are not set");
  }
...
double PairSPHMigrationSurfaceReactionSimpleCharge::single(int i, int j, int itype, int jtype,
...
int PairSPHMigrationSurfaceReactionSimpleCharge::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHMigrationSurfaceReactionSimpleCharge::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHMigrationSurfaceReactionSimpleCharge::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHMigrationSurfaceReactionSimpleCharge::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
