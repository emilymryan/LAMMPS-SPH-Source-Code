...
#include <stdio.h>
#include <string.h>
#include "fix_sph_surfacereaction_Langmuir.h"
...
FixSPHSurfaceReactionLangmuir::FixSPHSurfaceReactionLangmuir(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {

  if ((atom->e_flag != 1) || (atom->rho_flag != 1))
    error->all(FLERR,
               "fix sph/surfacereaction/Langmuir command requires atom_style with both energy and density, e.g. meso");

  if (narg != 5)
    error->all(FLERR,"Illegal number of arguments for fix sph/surfacereaction/Langmuir command");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for fix sph/surfacereaction/Langmuir");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for fix sph/surfacereaction/Langmuir");
  ny = atom->dvector[iny];
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for fix sph/surfacereaction/Langmuir");
  nz = atom->dvector[inz];
  ...
  if (ixA < 0)
    error->all(FLERR,
               "Can't find property xA for fix sph/surfacereaction/Langmuir");
  xA = atom->dvector[ixA];
  ...
  if (idxA < 0)
    error->all(FLERR,
               "Can't find property dxA for fix sph/surfacereaction/Langmuir");
  ...
  if (iyA < 0)
    error->all(FLERR,
               "Can't find property yA for fix sph/surfacereaction/Langmuir");
  yA = atom->dvector[iyA];
  ...
  if (idyA < 0)
    error->all(FLERR,
               "Can't find property dyA for fix sph/surfacereaction/Langmuir");
  ...
  if (ithetaA < 0)
    error->all(FLERR,
               "Can't find property thetaA for fix sph/surfacereaction/Langmuir");
...
void FixSPHSurfaceReactionLangmuir::init() {
  dtxA = update->dt;
...
void FixSPHSurfaceReactionLangmuir::init_list(int, NeighList *ptr) {
  list = ptr;
}

/* ---------------------------------------------------------------------- */

void FixSPHSurfaceReactionLangmuir::initial_integrate(int /*vflag*/)
{
...
void FixSPHSurfaceReactionLangmuir::reset_dt() {
  dtxA = update->dt;
}
