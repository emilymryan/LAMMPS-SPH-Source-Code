...
AtomVecMeso::AtomVecMeso(LAMMPS *lmp) : AtomVec(lmp)
{
  ...
  size_forward = 8; // 3 + rho + esph + vest[3], that means we may only communicate 5 in hybrid
  size_reverse = 5; // 3 + drho + desph
  size_border = 12; // 6 + rho + esph + vest[3] + cv
  size_velocity = 3;
  size_data_atom = 8;
  size_data_vel = 4;
  xcol_data = 6;
...
double AtomVecMeso::memory_usage() {
  double bytes = 0;
