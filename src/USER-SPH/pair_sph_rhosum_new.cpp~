...
PairSPHRhoSumNew::PairSPHRhoSumNew(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
...
PairSPHRhoSumNew::~PairSPHRhoSumNew() {
  if (allocated) {
...
void PairSPHRhoSumNew::init_style() {
...
void PairSPHRhoSumNew::compute(int eflag, int vflag) {
...
void PairSPHRhoSumNew::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHRhoSumNew::settings(int narg, char **arg) {
  if (narg != 1)
    error->all(FLERR,
...
void PairSPHRhoSumNew::coeff(int narg, char **arg) {
  if (narg != 3)
...
double PairSPHRhoSumNew::init_one(int i, int j) {
  if (setflag[i][j] == 0) {
...
void PairSPHRhoSumNew::unpack_comm(int n, int first, double *buf) {
  int i, m, last;
  double *rho = atom->rho;
