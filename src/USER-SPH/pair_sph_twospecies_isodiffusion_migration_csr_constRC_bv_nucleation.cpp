#include <math.h>
#include <stdlib.h>
#include "pair_sph_twospecies_isodiffusion_migration_csr_constRC_bv_nucleation.h"
...
PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  DA = atom->dvector[iDA];
  ...
  if (iDC < 0)
    error->all(FLERR,
	       "Can't find property DC for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  DC = atom->dvector[iDC];
  ...
  if (isurf_energy < 0)
    error->all(FLERR,
	       "Can't find property surf_energy sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (irR < 0)
    error->all(FLERR,
               "Can't find property rR for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  rR = atom->dvector[irR];
  ...
  if (idrR < 0)
    error->all(FLERR,
               "Can't find property drR for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (inN < 0)
  error->all(FLERR,
  	       "Can't find property nN for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  nN = atom->dvector[inN];
  ...
  if (idnN < 0)
  error->all(FLERR,
  	       "Can't find property dnN for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  mM = atom->dvector[imM];
  ...
  if (idmM < 0)
    error->all(FLERR,
	       "Can't find property dmM for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  ny = atom->dvector[iny];
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
  nz = atom->dvector[inz];
...
PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::~PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1)) { // fluid-fluid interaction outside of flux
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    i0 = faraday*RC*bulkcC*bulkcA*pow(10,-6)*pow(10,-9); //Exchange Current Density                                                                              
            Sc = 2*pi*pow(rc,2);
            omega = Sc*i0/e;
	    ...
	    deltaDcC = 1.0*RC*nN[j]*bulkcA*bulkcC*(((cC[i]/cC_init)*exp(transCoefC*(faraday*overPot/R/T + surf_energy[j]*2*mVol2/rR[j]/R/T))) - (exp(-1*transCoefM*(faraday*overPot/R/T + surf_energy[j]*2*mVol2/rR[j]/R/T))));
	  ...
	  if (r<= phase_support[itype][jtype]) {
	    ...
	    // Radius Calculation
	    deltaDrR = (RC*bulkcA*bulkcC*mVol*pow(10,-15))*(((cC[j]/cC_init)*exp(transCoefC*(faraday*overPot/R/T + surf_energy[i]*2*mVol2/rR[i]/R/T))) - (exp(-1*transCoefM*(faraday*overPot/R/T + surf_energy[i]*2*mVol2/rR[i]/R/T))));
	    drR[i] = deltaDrR;
	    ...
	    deltaDmM = (jmass)*RC*nN[j]*bulkcA*bulkcC*(((cC[j]/cC_init)*exp(transCoefC*(faraday*overPot/R/T + surf_energy[i]*2*mVol2/rR[i]/R/T))) - (exp(-1*transCoefM*(faraday*overPot/R/T + surf_energy[i]*2*mVol2/rR[i]/R/T))));
...
void PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation");
}
...
void PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::coeff(int narg, char **arg) {
  if (narg != 11)
    error->all(FLERR,"Incorrect number of args for pair_style sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation coefficients");
  if (!allocated)
    allocate();
  ...
  // Get the value of the reaction rate
  RC = utils::numeric(FLERR,arg[10],false,lmp);
...
double PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/twospecies/isodiffusion/migration/csr/constRC/bv/nucleation coeffs are not set");
  }
...
double PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRconstRCbvnucleation::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
