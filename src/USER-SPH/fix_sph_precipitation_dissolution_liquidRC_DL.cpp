...
#include <stdio.h>
#include <string.h>
#include "fix_sph_precipitation_dissolution_liquidRC_DL.h"
#include <math.h>
#include <stdlib.h>
#include <string.h>
#include "atom.h"
...
FixSPHPrecipitationDissolutionLiquidRCDL::FixSPHPrecipitationDissolutionLiquidRCDL(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {

  if ((atom->esph_flag != 1) || (atom->rho_flag != 1))
    error->all(FLERR,
  	       "fix sph/precipitation/dissolution/liquidRC/DL command requires atom_style with both energy and density, e.g. meso");

  if (narg != 10)
    error->all(FLERR,"Illegal number of arguments for fix sph/precipitation/dissolution/liquidRC/DL command");
  ...
  if (debyeE <= 0)
    error->all(FLERR,"Illegal value for debyeE length");
  if (debyeL <= 0)
    error->all(FLERR,"Illegal value for debyeL length");
  if (DDL <= 0)
    error->all(FLERR,"Illegal value for diffusion in the debye length");
  if (muDL <= 0)
    error->all(FLERR,"Illegal value for migration in the debye length");
// find the concentration property
  int fcC;
  int icC = atom->find_custom("cC", fcC);
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for fix sph/precipitation/dissolution/liquidRC/DL");
  cC = atom->dvector[icC];
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for fix sph/precipitation/dissolution/liquidRC/DL");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for fix sph/precipitation/dissolution/liquidRC/DL");
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for fix sph/precipitation/dissolution/liquidRC/DL");
  mM = atom->dvector[imM];
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for fix sph/precipitation/dissolution/liquidRC/DL");
  ...
  if (iDC < 0)
   error->all(FLERR,
  	        "Can't find property DC for fix sph/precipitation/dissolution/liquidRC/DL");
  DC = atom->dvector[iDC];
  ...
  if (iDA < 0)
    error->all(FLERR,
  	       "Can't find property DA for fix sph/precipitation/dissolution/liquidRC/DL");
  DA = atom->dvector[iDA];
  ...
  if (imuA < 0)
    error->all(FLERR,
               "Can't find property muA for fix sph/precipitation/dissolution/liquidRC/DL");
  ...
  if (imuC < 0)
    error->all(FLERR,
               "Can't find property muC for fix sph/precipitation/dissolution/liquidRC/DL");
  muC = atom->dvector[imuC];

  // find the SEI property
  int fsei;
  int isei = atom->find_custom("sei", fsei);
  if (isei < 0)
    error->all(FLERR,
               "Can't find property sei for fix sph/precipitation/dissolution/liquidRC/DL");
  sei = atom->dvector[isei];
...
FixSPHPrecipitationDissolutionLiquidRCDL::~FixSPHPrecipitationDissolutionLiquidRCDL() {
}

/* ---------------------------------------------------------------------- */

int FixSPHPrecipitationDissolutionLiquidRCDL::setmask() {
  int mask = 0;
  mask |= END_OF_STEP;
  return mask;
}
...
void FixSPHPrecipitationDissolutionLiquidRCDL::init() {
...
void FixSPHPrecipitationDissolutionLiquidRCDL::init_list(int, NeighList *ptr)
{
  list = ptr;
}
...
void FixSPHPrecipitationDissolutionLiquidRCDL::end_of_step()
{
  int i, j, ii, jj, itype, jtype, jshortest, inum, jnum, knum;
  int *ilist, *jlist, *numneigh, **firstneigh;

  double delx, dely, delz, delx_k, dely_k, delz_k;
  double shortest, rsq, r, r_k, cAdist;
  ...
  bool foundShortest, foundSolid, propogateFromSolid;
  ...
  for (ii=0; ii<inum; ii++) {
	...
	if ((foundShortest)) { // In electrolyte
	  ...
	  for (jj = 0; jj < jnum; jj++) { 
	    ...
	    // if particle is within the debye length, change the diffusion properties
	    if(jtype==1 and j!=jshortest and r <= debyeL) {
	      DC[j] = DDL;
	      DA[j] = DDL;
	      muC[j] = muDL;
	      muA[j] = muDL;
    ...
    } else if ((itype == 1 and DC[i] != DDL) or (itype == 1 and muC[i] != muDL) or (itype == 1 and sei[i] > 0)) { // if atom is liquid, check if solid is within debyeL 
      jnum = numneigh[i];
      jlist = firstneigh[i];
      foundSolid = false;
      //nearSolid = false;
      //propogateFromSolid = false;
      for (jj = 0; jj < jnum; jj++) {
	j = jlist[jj];
	j &= NEIGHMASK;
	jtype = type[j];
	...
	if (jtype == 2 and r <= debyeL) {
	  foundSolid = true;
	  sei[i] = 1;
	  break;
	}

	if(jtype == 1 and r <= debyeE and sei[j] == 1 and sei[i] == 0) {
	  //propogateFromSolid = true;
	  //break;
	  sei[i] = 2;
	}

	//if(jtype == 1 and r <= debyeE and sei[j] == 2 and sei[i] == ) {
          //propogateFromSolid = true;                                                                                                                                               
          //break;                                                                                                                                                                   
          //sei[i] = 3;
        //}
      ...
      if ((foundSolid || sei[i] > 0)) { // || propogateFromSolid)) { 
	DC[i] = DDL;
	DA[i] = DDL;
	muC[i] = muDL;
	muA[i] = muDL;
...
int FixSPHPrecipitationDissolutionLiquidRCDL::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i, j, m;
  m = 0;
  for (i = 0; i < n; i++) {
...
void FixSPHPrecipitationDissolutionLiquidRCDL::unpack_forward_comm(int n, int first, double *buf) 
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int FixSPHPrecipitationDissolutionLiquidRCDL::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  int *type = atom->type;
...
void FixSPHPrecipitationDissolutionLiquidRCDL::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  int *type = atom->type;
