#include <math.h>
#include <stdlib.h>
#include "pair_sph_twospecies_isodiffusion_migration_csr_bv_nucleation_hybrid.h"
...
PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  DA = atom->dvector[iDA];
  ...
  if (iDC < 0)
    error->all(FLERR,
	       "Can't find property DC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  DC = atom->dvector[iDC];
  ...
  if (iRC < 0)
    error->all(FLERR,
               "Can't find property RC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  RC = atom->dvector[iRC];
  ...
  if (isurf_energy < 0)
    error->all(FLERR,
	       "Can't find property surf_energy sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (irR < 0)
    error->all(FLERR,
               "Can't find property rR for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  rR = atom->dvector[irR];
  ...
  if (idrR < 0)
    error->all(FLERR,
               "Can't find property drR for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (inN < 0)
  error->all(FLERR,
  	       "Can't find property nN for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  nN = atom->dvector[inN];
  ...
  if (idnN < 0)
  error->all(FLERR,
  	       "Can't find property dnN for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  mM = atom->dvector[imM];
  ...
  if (idmM < 0)
    error->all(FLERR,
	       "Can't find property dmM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  ny = atom->dvector[iny];
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
  nz = atom->dvector[inz];
...
PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::~PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1) ||
	    (itype==1) && (jtype==3) ||
	    (itype==3) && (jtype==1) ||
	    (itype==3) && (jtype==3)) { // fluid-fluid interaction outside of flux
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    i0 = faraday*RC[i]*bulkcC*bulkcA*pow(10,-6)*pow(10,-9); //Exchange Current Density                                                                              
            Sc = 2*pi*pow(rc,2);
            omega = Sc*i0/e;
            gc = (32*pi/3)*rc*rc*rc*0.5*Na/mVol;
            dGc = fabs(overPot - applied_pot);
            Z = pow(dGc/(3*pi*Kb*T),0.5)/gc;
            deltaDnN = Nj*omega*Z*exp(-dGc/(Kb*T));
            //printf("deltaDnN: %.10f \n", deltaDnN);                                                                                                                    
            dnN[i] = deltaDnN; //deltaDnN;
	    
	    // Butler - Volmer equations for reaction at solid fluid interface
	    deltaDcC = 1.0*RC[j]*nN[i]*bulkcA*bulkcC*(((cC[i]/cC_init)*exp(transCoefC*(faraday*overPot/R/T + surf_energy[j]*2*mVol2/rR[j]/R/T))) - (exp(-1*transCoefM*(faraday*overPot/R/T + surf_energy[j]*2*mVol2/rR[j]/R/T))));
	  ...
	  if (r<= phase_support[itype][jtype]) {
	    ...
	    deltaDmM = (jmass)*RC[i]*nN[j]*bulkcA*bulkcC*(((cC[j]/cC_init)*exp(transCoefC*(faraday*overPot/R/T + surf_energy[i]*2*mVol2/rR[i]/R/T))) - (exp(-1*transCoefM*(faraday*overPot/R/T + surf_energy[i]*2*mVol2/rR[i]/R/T))));
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
}
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::coeff(int narg, char **arg) {
  if (narg != 10)
    error->all(FLERR,"Incorrect number of args for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid coefficients");
  if (!allocated)
    allocate();
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid");
}
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/twospecies/isodiffusion/migration/csr/bv/nucleation/hybrid coeffs are not set");
  }
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnucleationhybrid::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
