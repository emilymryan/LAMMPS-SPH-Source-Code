...
#include <stdio.h>
#include <string.h>
#include "fix_sph_precipitation_dissolution_liquidRC_SE.h"
#include <math.h>
#include <stdlib.h>
#include <string.h>
#include "atom.h"
...
FixSPHPrecipitationDissolutionLiquidRCSE::FixSPHPrecipitationDissolutionLiquidRCSE(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {

  if ((atom->esph_flag != 1) || (atom->rho_flag != 1))
    error->all(FLERR,
  	       "fix sph/precipitation/dissolution/liquidRC/SE command requires atom_style with both energy and density, e.g. meso");

  if (narg != 6)
    error->all(FLERR,"Illegal number of arguments for fix sph/precipitation/dissolution/liquidRC/SE command");
  ...
  //if (SE <= 0)
  //error->all(FLERR, "Illegal value for parent surface energy");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for fix sph/precipitation/dissolution/liquidRC/SE");
  cC = atom->dvector[icC];
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for fix sph/precipitation/dissolution/liquidRC/SE");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for fix sph/precipitation/dissolution/liquidRC/SE");
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for fix sph/precipitation/dissolution/liquidRC/SE");
  mM = atom->dvector[imM];
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for fix sph/precipitation/dissolution/liquidRC/SE");
  ...
  if (isurf_energy < 0)
    error->all(FLERR,
	       "Can't find property surf_energy for fix sph/precipitation/dissolution/liquidRC/SE");
...
FixSPHPrecipitationDissolutionLiquidRCSE::~FixSPHPrecipitationDissolutionLiquidRCSE() {
}

/* ---------------------------------------------------------------------- */

int FixSPHPrecipitationDissolutionLiquidRCSE::setmask() {
  int mask = 0;
  mask |= END_OF_STEP;
  return mask;
}
...
void FixSPHPrecipitationDissolutionLiquidRCSE::init() {
...
void FixSPHPrecipitationDissolutionLiquidRCSE::init_list(int, NeighList *ptr)
{
  list = ptr;
}
...
void FixSPHPrecipitationDissolutionLiquidRCSE::end_of_step()
{
...
int FixSPHPrecipitationDissolutionLiquidRCSE::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i, j, m;
  m = 0;
  for (i = 0; i < n; i++) {
...
void FixSPHPrecipitationDissolutionLiquidRCSE::unpack_forward_comm(int n, int first, double *buf) 
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int FixSPHPrecipitationDissolutionLiquidRCSE::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  int *type = atom->type;
...
void FixSPHPrecipitationDissolutionLiquidRCSE::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  int *type = atom->type;
