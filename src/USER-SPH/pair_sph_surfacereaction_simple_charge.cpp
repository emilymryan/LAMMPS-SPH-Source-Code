...
PairSPHSurfaceReactionSimpleCharge::PairSPHSurfaceReactionSimpleCharge(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/surfacereaction/simple/charge");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/surfacereaction/simple/charge");
  dcA = atom->dvector[idcA];

  // find the local diffusivity constant x
  int fDAx;
  int iDAx = atom->find_custom("DAx", fDAx);
  if (iDAx < 0)
    error->all(FLERR,
	       "Can't find property DAx for pair_style sph/surfacereaction/simple/charge");
  ...
  if (iDAy < 0)
    error->all(FLERR,
	       "Can't find property DAy for pair_style sph/surfacereaction/simple/charge");
  ...
  if (idmA < 0)
    error->all(FLERR,
	       "Can't find property dmA for pair_style sph/surfacereaction/simple/charge");
  ...
  if (imA < 0)
    error->all(FLERR,
	       "Can't find property mA for pair_style sph/surfacereaction/simple/charge");
  mA = atom->dvector[imA];
...
PairSPHSurfaceReactionSimpleCharge::~PairSPHSurfaceReactionSimpleCharge() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHSurfaceReactionSimpleCharge::init_style()
{
...
void PairSPHSurfaceReactionSimpleCharge::compute(int eflag, int vflag) {
...
void PairSPHSurfaceReactionSimpleCharge::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHSurfaceReactionSimpleCharge::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
...
void PairSPHSurfaceReactionSimpleCharge::coeff(int narg, char **arg) {
  if (narg != 7)
    error->all(FLERR,"Incorrect number of args for pair_style sph/surfacereaction/simple/charge coefficients");
  if (!allocated)
    allocate();
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/surfacereaction/simple/charge");
}
...
double PairSPHSurfaceReactionSimpleCharge::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/surfacereaction/simple/charge coeffs are not set");
  }
...
double PairSPHSurfaceReactionSimpleCharge::single(int i, int j, int itype, int jtype,
...
int PairSPHSurfaceReactionSimpleCharge::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHSurfaceReactionSimpleCharge::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHSurfaceReactionSimpleCharge::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHSurfaceReactionSimpleCharge::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
