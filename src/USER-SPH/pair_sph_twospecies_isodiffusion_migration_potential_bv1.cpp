...
PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::PairSPHTwospeciesIsodiffusionMigrationPotentialBV1(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  DA = atom->dvector[iDA];
  ...
  if (iDC < 0)
    error->all(FLERR,
	       "Can't find property DC for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  DC = atom->dvector[iDC];
  ...
  if (iRC < 0)
    error->all(FLERR,
	       "Can't find property RC for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  RC = atom->dvector[iRC];
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  mM = atom->dvector[imM];
  ...
  if (idmM < 0)
    error->all(FLERR,
	       "Can't find property dmM for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (inext_local_pot < 0)
    error->all(FLERR,
	       "Can't find property next_local_pot for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  nx = atom->dvector[inx];
  ...
  if (idnx < 0)
    error->all(FLERR,
	       "Can't find property dnx for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ny = atom->dvector[iny];
  ...
  if (idny < 0)
    error->all(FLERR,
	       "Can't find property dny for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
  nz = atom->dvector[inz];
  ...
  if (idnz < 0)
    error->all(FLERR,
	       "Can't find property dnz for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
...
PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::~PairSPHTwospeciesIsodiffusionMigrationPotentialBV1() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::compute(int eflag, int vflag) {
  ...
  double deltaDmM;
  double chargedensityi, chargedensityj, anodePot, overPot;
  ...
  for (ii = 0; ii < inum; ii++)
    {
      ...
      dny[ii] = 0.0;
      dnz[ii] = 0.0;
      anodePot += log(mM[ii]); // Nearnst equation
    }
  
  anodePot *= R*T/faraday;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1)) { // fluid-fluid interaction outside of flux
	  ...
	  if (muA[i]>0.0||muC[i]>0.0){
	    // Migration                                                                                  
	    deltaMcAi = muA[i]*cA[i]/0.213532*0.5*ni*imass;
	    deltaMcAj = muA[i]*cA[j]/0.213532*0.5*nj*jmass; // TODO: muA[j] is always 0 when it should be    equal to muA[i]                                                                                        
	    // Migration                                                                               
	    deltaMcCi = muC[i]*cC[i]/0.213532*0.5*ni*imass;
	    deltaMcCj = muC[i]*cC[j]/0.213532*0.5*nj*jmass;// TODO: muA[j] is always 0 when it should be equal to muA[i]                                                                                         
          }	  
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    deltaDcC = 1.0*RC[j]*(cC[i]*exp(transCoefC*faraday*overPot/R/T) - mM[j]*exp(-1*transCoefM*faraday*overPot/R/T));
	    dcC[i] -= deltaDcC;
	    printf("deltaDcC: %2.8f \n", deltaDcC);
	  }
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    // Butler - Volmer equations for reaction at solid fluid interface
	    deltaDmM = (jmass)*RC[i]*(cC[j]*exp(transCoefC*faraday*overPot/R/T) - mM[i]*exp(-1*transCoefM*faraday*overPot/R/T));
	    dmM[i] += deltaDmM;
...
void PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/twospecies/isodiffusion/migration/potential/bv1");
}
...
void PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::coeff(int narg, char **arg) {
  if (narg != 9)
    error->all(FLERR,"Incorrect number of args for pair_style sph/twospecies/isodiffusion/migration/potential/bv1  coefficients");
  if (!allocated)
    allocate();
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/twospecies/isodiffusion/migration/potential/bv1");
}
...
double PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/twospecies/isodiffusion/migration/potential/bv1 coeffs are not set");
  }
...
double PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationPotentialBV1::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
