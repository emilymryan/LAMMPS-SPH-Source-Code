...
#include <stdio.h>
#include <string.h>
#include "fix_sph_surfacereaction_simple_precipitation_discharge.h"
#include <math.h>
#include <stdlib.h>
#include <string.h>
#include "atom.h"
...
FixSPHSurfaceReactionSimplePrecipitationDischarge::FixSPHSurfaceReactionSimplePrecipitationDischarge(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {

  if ((atom->esph_flag != 1) || (atom->rho_flag != 1))
    error->all(FLERR,
	       "fix sph/surfacereaction/simple/precipitation/discharge command requires atom_style with both energy and density, e.g. meso");

  if (narg != 6)
    error->all(FLERR,"Illegal number of arguments for fix sph/surfacereaction/simple/precipitation/discharge command");
...
FixSPHSurfaceReactionSimplePrecipitationDischarge::~FixSPHSurfaceReactionSimplePrecipitationDischarge() {
}

/* ---------------------------------------------------------------------- */

int FixSPHSurfaceReactionSimplePrecipitationDischarge::setmask() {
  int mask = 0;
  mask |= END_OF_STEP;
  return mask;
}
...
void FixSPHSurfaceReactionSimplePrecipitationDischarge::init() {
...
void FixSPHSurfaceReactionSimplePrecipitationDischarge::init_list(int, NeighList *ptr)
{
  list = ptr;
}
...
void FixSPHSurfaceReactionSimplePrecipitationDischarge::end_of_step()
{
  ...
  for (ii=0; ii<inum; ii++) {
	...
	if ((foundShortest)) { // In electrolyte
      ...
      else if ((mA[i] <= -mAthres)) { // Dissolution in electrolyte
	ischangecA[i] += 1.0;
      }
    }
  }
  ...
  for (i = 0; i < nlocal; i++) {
    if (ischangecA[i] > 0.01) {
      ...
      if (type[i] == 1) {
      ...
      else if (type[i] == 2 && (x[i][0] > 0.01)) { // In electrolyte
        ...
        else if (mA[i] >= mAthres && (x[i][0] > 0.01)) { // In electrolyte
...
int FixSPHSurfaceReactionSimplePrecipitationDischarge::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  int *type = atom->type;
...
void FixSPHSurfaceReactionSimplePrecipitationDischarge::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  int *type = atom->type;
