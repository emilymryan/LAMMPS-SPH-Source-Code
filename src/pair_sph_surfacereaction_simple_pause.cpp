...
PairSPHSurfaceReactionSimplePause::PairSPHSurfaceReactionSimplePause(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/surfacereaction/simple/pause");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/surfacereaction/simple/pause");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/surfacereaction/simple/pause");
  DA = atom->dvector[iDA];
  ...
  if (idmA < 0)
    error->all(FLERR,
	       "Can't find property dmA for pair_style sph/surfacereaction/simple/pause");
  ...
  if (imA < 0)
    error->all(FLERR,
	       "Can't find property mA for pair_style sph/surfacereaction/simple/pause");
  mA = atom->dvector[imA];
...
PairSPHSurfaceReactionSimplePause::~PairSPHSurfaceReactionSimplePause() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHSurfaceReactionSimplePause::init_style()
{
...
void PairSPHSurfaceReactionSimplePause::compute(int eflag, int vflag) {
...
void PairSPHSurfaceReactionSimplePause::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHSurfaceReactionSimplePause::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/surfacereaction/simple/pause");
}
...
void PairSPHSurfaceReactionSimplePause::coeff(int narg, char **arg) {
  if (narg != 7)
    error->all(FLERR,"Incorrect number of args for pair_style sph/surfacereaction/simple/pause coefficients");
  if (!allocated)
    allocate();
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/surfacereaction/simple/pause");
}
...
double PairSPHSurfaceReactionSimplePause::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/surfacereaction/simple/pause coeffs are not set");
  }
...
double PairSPHSurfaceReactionSimplePause::single(int i, int j, int itype, int jtype,
...
int PairSPHSurfaceReactionSimplePause::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHSurfaceReactionSimplePause::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHSurfaceReactionSimplePause::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHSurfaceReactionSimplePause::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
