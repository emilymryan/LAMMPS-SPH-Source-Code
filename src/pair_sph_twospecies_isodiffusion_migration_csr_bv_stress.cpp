...
PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::PairSPHTwospeciesIsodiffusionMigrationCSRbvstress(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  DA = atom->dvector[iDA];
  ...
  if (iDC < 0)
    error->all(FLERR,
	       "Can't find property DC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  DC = atom->dvector[iDC];
  ...
  if (iRC < 0)
    error->all(FLERR,
               "Can't find property RC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  RC = atom->dvector[iRC];
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  mM = atom->dvector[imM];
  ...
  if (idmM < 0)
    error->all(FLERR,
	       "Can't find property dmM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  ny = atom->dvector[iny];
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
  nz = atom->dvector[inz];
...
PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::~PairSPHTwospeciesIsodiffusionMigrationCSRbvstress() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::compute(int eflag, int vflag) {
  ...
  double transCoefC, transCoefM, transCoefA;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1)) { // fluid-fluid interaction outside of flux
	  ...
	  //	  if(cC[i] < 0){
	  // printf("%s \n", "cC is negative, exit simulation");
	  // abort();
	  //  }
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    deltaDcC = 1.0*RC[j]*bulkcA*bulkcC*(((cC[i]/cC_init)*exp((transCoefC*faraday*overPot/R/T)+(transCoefM*faraday*stressPot/R/T))) - (exp(-1*transCoefA*faraday*overPot/R/T)));
	  ...
	  if (r<= phase_support[itype][jtype]) {
	    ...
	    deltaDmM = (jmass)*RC[i]*bulkcA*bulkcC*(((cC[j]/cC_init)*exp((transCoefC*faraday*overPot/R/T)+(transCoefM*faraday*stressPot/R/T))) - (exp(-1*transCoefA*faraday*overPot/R/T)));
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress");
}
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::coeff(int narg, char **arg) {
  if (narg != 9)
    error->all(FLERR,"Incorrect number of args for pair_style sph/twospecies/isodiffusion/migration/csr/bv/stress coefficients");
  if (!allocated)
    allocate();
  ...
  // Get the potential resuling from the stress at the interface
  stressPot = utils::numeric(FLERR,arg[8],false,lmp);
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/twospecies/isodiffusion/migration/csr/bv/stress");
}
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/twospecies/isodiffusion/migration/csr/bv/stress coeffs are not set");
  }
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvstress::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
