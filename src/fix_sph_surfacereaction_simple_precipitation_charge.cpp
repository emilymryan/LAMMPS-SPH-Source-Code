...
#include <stdio.h>
#include <string.h>
#include "fix_sph_surfacereaction_simple_precipitation_charge.h"
#include <math.h>
#include <stdlib.h>
#include <string.h>
#include "atom.h"
...
FixSPHSurfaceReactionSimplePrecipitationCharge::FixSPHSurfaceReactionSimplePrecipitationCharge(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {

  if ((atom->esph_flag != 1) || (atom->rho_flag != 1))
    error->all(FLERR,
	       "fix sph/surfacereaction/simple/precipitation/charge command requires atom_style with both energy and density, e.g. meso");

  if (narg != 6)
    error->all(FLERR,"Illegal number of arguments for fix sph/surfacereaction/simple/precipitation/charge command");
...
FixSPHSurfaceReactionSimplePrecipitationCharge::~FixSPHSurfaceReactionSimplePrecipitationCharge() {
}

/* ---------------------------------------------------------------------- */

int FixSPHSurfaceReactionSimplePrecipitationCharge::setmask() {
  int mask = 0;
  mask |= END_OF_STEP;
  return mask;
}
...
void FixSPHSurfaceReactionSimplePrecipitationCharge::init() {
...
void FixSPHSurfaceReactionSimplePrecipitationCharge::init_list(int, NeighList *ptr)
{
  list = ptr;
}
...
void FixSPHSurfaceReactionSimplePrecipitationCharge::end_of_step()
{
  ...
  for (i = 0; i < nlocal; i++) {
    if (ischangecA[i] > 0.01) {
      ...
      if (type[i] == 1) {
        ...
        else if (mA[i] >= mAthres) { // In electrolyte
...
int FixSPHSurfaceReactionSimplePrecipitationCharge::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  int *type = atom->type;
...
void FixSPHSurfaceReactionSimplePrecipitationCharge::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  int *type = atom->type;
