...
PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (icA < 0)
    error->all(FLERR,
	       "Can't find property cA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  cA = atom->dvector[icA];
  ...
  if (idcA < 0)
    error->all(FLERR,
	       "Can't find property dcA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ...
  if (icC < 0)
    error->all(FLERR,
	       "Can't find property cC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  cC = atom->dvector[icC];
  ...
  if (idcC < 0)
    error->all(FLERR,
	       "Can't find property dcC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ...
  if (imuA < 0)
    error->all(FLERR,
	       "Can't find property muA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ...
  if (imuC < 0)
    error->all(FLERR,
	       "Can't find property muC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ...
  if (iDA < 0)
    error->all(FLERR,
	       "Can't find property DA for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  DA = atom->dvector[iDA];
  ...
  if (iDC < 0)
    error->all(FLERR,
	       "Can't find property DC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  DC = atom->dvector[iDC];
  ...
  if (iRC < 0)
    error->all(FLERR,
               "Can't find property RC for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  RC = atom->dvector[iRC];
  ...
  if (isurf_energy < 0)
    error->all(FLERR,
	       "Can't find property surf_energy sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ...
  if (irR < 0)
    error->all(FLERR,
               "Can't find property rR for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  rR = atom->dvector[irR];
  ...
  if (idrR < 0)
    error->all(FLERR,
               "Can't find property drR for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ...
  if (imM < 0)
    error->all(FLERR,
	       "Can't find property mM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  mM = atom->dvector[imM];
  ...
  if (idmM < 0)
    error->all(FLERR,
	       "Can't find property dmM for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ...
  if (ilocal_pot < 0)
    error->all(FLERR,
	       "Can't find property local_pot for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  ny = atom->dvector[iny];
  ...
  if (inz < 0)
    error->all(FLERR,
	       "Can't find property nz for pair_style sph/twospecies/isodiffusion/migration/csr/bv/nuc");
  nz = atom->dvector[inz];
...
PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::~PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1)) { // fluid-fluid interaction outside of flux
	  ...
	  if (r<= phase_support[itype][jtype]) {
	    ...
	    deltaDmM = (jmass)*RC[i]*bulkcA*bulkcC*(((cC[j]/cC_init)*exp(transCoefC*(faraday*overPot/R/T + surf_energy[i]*2*mVol/rR[i]/R/T))) - ((cA[j]/cA_init)*exp(-1*transCoefM*(faraday*overPot/R/T + surf_energy[i]*2*mVol/rR[i]/R/T))));
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::coeff(int narg, char **arg) {
  if (narg != 8)
  ...
  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients sph/twospecies/isodiffusion/migration/csr/constrc/bv/nuc");
}
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/twospecies/isodiffusion/migration/csr/bv/nuc coeffs are not set");
  }
...
double PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationCSRbvnuc::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
