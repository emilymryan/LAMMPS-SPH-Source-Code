...
PairSPHTwospeciesIsodiffusionMigrationCSR::PairSPHTwospeciesIsodiffusionMigrationCSR(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (iRC < 0)
    error->all(FLERR,
	       "Can't find property RC for pair_style sph/twospecies/isodiffusion/migration/csr");
  RC = atom->dvector[iRC];
  ...
  if (inx < 0)
    error->all(FLERR,
	       "Can't find property nx for pair_style sph/twospecies/anisodiffusion/migration/csr");
  nx = atom->dvector[inx];
  ...
  if (iny < 0)
    error->all(FLERR,
	       "Can't find property ny for pair_style sph/twospecies/anisodiffusion/migration/csr");
  ny = atom->dvector[iny];
...
PairSPHTwospeciesIsodiffusionMigrationCSR::~PairSPHTwospeciesIsodiffusionMigrationCSR() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHTwospeciesIsodiffusionMigrationCSR::init_style()
{
...
void PairSPHTwospeciesIsodiffusionMigrationCSR::compute(int eflag, int vflag) {
...
void PairSPHTwospeciesIsodiffusionMigrationCSR::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
double PairSPHTwospeciesIsodiffusionMigrationCSR::single(int i, int j, int itype, int jtype,
...
int PairSPHTwospeciesIsodiffusionMigrationCSR::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHTwospeciesIsodiffusionMigrationCSR::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
int PairSPHTwospeciesIsodiffusionMigrationCSR::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void PairSPHTwospeciesIsodiffusionMigrationCSR::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
